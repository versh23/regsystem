...
class public_forums_forums_topics extends ipsCommand
{
	...
	public function loadTopicAndForum( $topic="" )
	{
		...
		$approved = $this->registry->class_forums->fetchHiddenTopicType( $this->topic );
		
		if ( ! $this->registry->class_forums->canQueuePosts($this->forum['id']) )
		{
			if ( $approved == 'hidden' )
			{
				$this->registry->output->showError( 'topic_not_approved', 10344, null, null, 403 );
			}
		}
		
		$this->permissions['softDelete']             = $this->registry->getClass('class_forums')->canSoftDeletePosts( $this->topic['forum_id'], array() );
		$this->permissions['softDeleteRestore']      = $this->registry->getClass('class_forums')->can_Un_SoftDeletePosts( $this->topic['forum_id'] );
		$this->permissions['softDeleteSee']          = $this->registry->getClass('class_forums')->canSeeSoftDeletedPosts( $this->topic['forum_id'] );
		$this->permissions['softDeleteReason']       = $this->registry->getClass('class_forums')->canSeeSoftDeleteReason( $this->topic['forum_id'] );
		$this->permissions['softDeleteContent']      = $this->registry->getClass('class_forums')->canSeeSoftDeleteContent( $this->topic['forum_id'] );
		$this->permissions['TopicSoftDelete']        = $this->registry->getClass('class_forums')->canSoftDeleteTopics( $this->forum['id'], $this->topic );
		...
		/* Soft deleted? */
		if ( $approved == 'sdelete' AND ! $this->permissions['softDeleteContent'] )
		{
			$this->registry->output->showError( 'topic_not_approved', 10344.1, null, null, 403 );
		}
		
		$this->registry->class_forums->forumsCheckAccess( $this->forum['id'], 1, 'topic', $this->topic );
		
		/* Got a nice perm? */
		$this->topic['permissions'] =& $this->permissions;
		
		/* Deleted topic? */
		$this->topic['_isDeleted'] = ( $approved == 'sdelete' ) ? true : false;
		$this->topic['_isHidden']  = ( $approved == 'hidden' )  ? true : false;
	}
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		{
			$classToLoad = IPSLib::loadLibrary( IPSLib::getAppDir( 'forums' ) . '/sources/classes/threaded.php', 'threadedModeLibrary', 'forums' );
			$threaded	= new $classToLoad( $this->registry, $this );
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $this->topic, 'topicViewTopicData' );
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $displayData, 'topicViewDisplayData' );
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $this->forum, 'topicViewForumData' );
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( $post_data, 'topicViewPostData' );
		...
		$template = $this->registry->output->getTemplate('topic')->topicViewTemplate( $this->forum, $this->topic, $post_data, $displayData, $ps_rank );
		 ...
		 */
		$this->registry->output->addToDocumentHead( 'raw', "<link rel='up' href='" . $this->registry->output->buildSEOUrl( 'showforum=' . $this->topic['forum_id'], 'publicNoSession', $this->forum['name_seo'], 'showforum' ) . "' />" );
		$this->registry->output->addToDocumentHead( 'raw', "<link rel='author' href='" . $this->registry->output->buildSEOUrl( 'showuser=' . $this->topic['starter_id'], 'publicNoSession', $this->topic['seo_first_name'], 'showuser' ) . "' />" );
	...
	public function parsePostRow( $row = array() )
	{
		...
		if ( $row['author_id'] != 0 )
		{
			...
			else
			{
				...
				unset( $poster['pid'], $poster['append_edit'], $poster['edit_time'], $poster['use_sig'], $poster['use_emo'], $poster['post_edit_reason'],
						$poster['post_date'], $poster['icon_id'], $poster['post'], $poster['queued'], $poster['topic_id'], $poster['post_htmlstate'],
						$poster['post_title'], $poster['new_topic'], $poster['edit_name'], $poster['post_key'], $poster['post_parent'] );
		...
		/* Update permission */
		$this->permissions['softDelete'] = $this->registry->getClass('class_forums')->canSoftDeletePosts( $this->topic['forum_id'], $row );
		
		/* Soft delete */
		$row['_softDelete']        = ( $row['pid'] != $this->topic['topic_firstpost'] ) ? $this->permissions['softDelete'] : FALSE;
		$row['_softDeleteRestore'] = $this->permissions['softDeleteRestore'];
		$row['_softDeleteSee']     = $this->permissions['softDeleteSee'];
		$row['_softDeleteReason']  = $this->permissions['softDeleteReason'];
		$row['_softDeleteContent'] = $this->permissions['softDeleteContent'];
		
		$row['_isVisible']		   = ( $this->registry->getClass('class_forums')->fetchHiddenType( $row ) == 'visible' ) ? true : false;
		$row['_isHidden']		   = ( $this->registry->getClass('class_forums')->fetchHiddenType( $row ) == 'hidden' ) ? true : false;
		$row['_isDeleted']		   = ( $this->registry->getClass('class_forums')->fetchHiddenType( $row ) == 'sdelete' ) ? true : false;
		
		/* Collect PIDS of soft deleted posts */
		if ( $row['_isDeleted'] AND $row['_softDeleteSee'] )
		{
			$this->_sdPids[] = $row['pid'];
		}
		...
		if ( $this->topic['topic_firstpost'] == $row['pid'] and ( $row['_isHidden'] OR $this->topic['_isHidden'] ) )
		{
			$row['queued']    = 1;
			$row['_isHidden'] = true;
		}
	...
	*/
	...
	public function _generatePollOutput()
	{
			...
			if ( $poll['poll_view_voters'] AND $voter['member_choices'] AND $this->settings['poll_allow_public'] )
			{
				...
				if ( is_array( $_choices ) AND count( $_choices ) )
				{
					$memberData = array( 'member_id'            => $voter['member_id'],
										 ...
										 'members_colored_name' => str_replace( '"', '\"', IPSLib::makeNameFormatted( $voter['members_display_name'], $voter['member_group_id'] ) ),
										 '_last'                => 0 );
		...
		$_editPoll	= $this->_getEditButtonData( array( 'member_id' => $this->topic['starter_id'], 'post_date' => $this->topic['start_date'] ) );

		$html = $this->registry->output->getTemplate('topic')->pollDisplay( $poll, $this->topic, $this->forum, $pollData, $showResults, $_editPoll );
	...
	public function _getReplyButtonData()
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if ( $this->memberData['member_id'] AND ( ( ( $this->memberData['member_id'] == $this->topic['starter_id'] ) AND ! $this->memberData['g_reply_own_topics'] ) OR ( ( $this->memberData['member_id'] != $this->topic['starter_id'] ) AND ! $this->memberData['g_reply_other_topics'] ) ) )
					{
						$image = "no_reply";
					}
	...
	public function _getFastReplyData()
	{
		...
		{
			/* status from PPD */
			if ( $this->registry->getClass('class_forums')->ppdStatusMessage )
			{
	...
	public function _getActiveUserData()
	{
		...
		{	
			...
			$this->DB->build( array( 
									'select' => 's.member_id, s.member_name, s.member_group, s.id, s.login_type, s.running_time, s.uagent_type, s.current_module, s.seo_name , s.uagent_key',
									'from'	 => 'sessions s',
			...
			/* (T31) Display Members Browser */
			if ( $this->settings['tb_dmb_enable'] && $this->settings['tb_dmb_topic'] )
			{
				require_once( IPS_ROOT_PATH . 'sources/classes/useragents/tbDisplayMembersBrowser.php' );
				$this->registry->setClass( 'tbDisplayMembersBrowser', new tbDisplayMembersBrowser( $this->registry ) );
			}
			...
			foreach( $rows as $result )
			{
				...
				$last_date = $this->registry->class_localization->getDate( $result['running_time'], 'TINY' );
				...
				else
				{
					if (empty( $cached[ $result['member_id'] ] ) )
					{
						...
						if ( $this->settings['tb_dmb_enable'] && $this->settings['tb_dmb_topic'] )
						{
							$result['member_name'] = $this->registry->getClass('tbDisplayMembersBrowser')->getImage( $result );
						}
	...
	public function _getTopicDataLinear()
	{
		...
		if ( $this->request['modfilter'] == 'deleted_posts' AND $this->permissions['softDeleteSee'] )
		{
		...
		while ( $row = $this->DB->fetch( $oq ) )
		{
			...
			//тут надо вычислять его ранк и передавать в массив 
			//вычисляем стим
			$this->DB->build( array( 'select' => 's.steam',
											'from' => array( 'srr_steam' => 's' ),
											'where' => 'h.forum_id='.$poster['mid'],
											'order'		=> 's.date DESC',
											'add_join'	=> array( array( 'select'	=> 'h.forum_id',
																		 'from'		=> array( 'srr_hdd' => 'h' ),
																		 'where'	=> 's.hddsn=h.hddsn'))));
			...
			$poster['ps_rank'] = $row_rank[0];
			$poster['steam'] = $row_steam['steam'];
	...
	public function topicSetUp()
	{
		...
		if ( $this->topic['state'] == 'link' )
		{
			...
			$this->registry->output->redirectScreen( $this->lang->words['topic_moved'], $this->settings['base_url'] . "showtopic={$f_stuff[0]}", $_topic['title_seo'], 'showtopic' );
		}
		...
		if ( $this->permissions['softDeleteSee'] AND $this->topic['topic_deleted_posts'] )
		{
			$this->topic['posts'] += intval( $this->topic['topic_deleted_posts'] );
		}
